...
from chronologer_unimod_tokenizer import residues


class chronologer_unimod_model(nn.Module):
    def __init__(self, vec_length, n_states, embed_dim, n_blocks, kernel, drop_rate, act_fx):
        ...
        self.resnet_blocks = nn.Sequential(
            *[
                resnet_block(embed_dim, embed_dim, kernel, d + 1, act_fx)
...
def initialize_chronologer_unimod_model(model_file=None, map_location='cpu'):
    model = chronologer_unimod_model(
        chronologer_max_peptide_len + 2,
        len(residues) + 1,
    ...
    if model_file:
        state = torch.load(model_file, map_location=torch.device(map_location))
