...
class ProspectChargeDataset( IterableDataset ):
	"""Iterable dataset for Prospect-PTMs charge state parquet shards."""
		...
		for filepath in files:
			...
			for rg_idx in range( pf.metadata.num_row_groups ):
				...
				for j in range( len(mod_seqs) ):
					...
					seq_arr = codedseq_to_array( coded, max_size=seq_size )
					dist_arr = np.asarray( charge_dists[j], 'float32' )
					weight_arr = np.asarray( [ 1.0 ], 'float32' )

					yield ( torch.from_numpy( seq_arr ),
					        torch.from_numpy( dist_arr ),
					        torch.from_numpy( weight_arr ), )
